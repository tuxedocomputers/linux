...
CURRENT_TUXEDO_VERSION_NUMBER=$(grep --perl-regexp --only-matching --max-count 1 "(?<=^linux-${TUXEDO_KERNEL_BRANCH_FULL} \().*(?=\))" debian.${TUXEDO_KERNEL_BRANCH}/changelog)
...
if [[ ${DRY} ]]; then
    ...
    echo "    DEBFULLNAME="Tuxedo BOT" DEBEMAIL="tux@tuxedocomputers.com" gbp dch --new-version \"${NEXT_TUXEDO_VERSION_NUMBER}\" --release --since \"${NEXT_BASE_TAG}\" --ignore-branch --spawn-editor \"never\""
    ...
    DEBFULLNAME="Tuxedo BOT" DEBEMAIL="tux@tuxedocomputers.com" gbp dch --new-version "${NEXT_TUXEDO_VERSION_NUMBER}" --release --since "${NEXT_BASE_TAG}" --ignore-branch --spawn-editor "never"
    if [[ "$UBUNTU_KERNEL_BRANCH" == "master" ]]; then
