...
NEWEST_TUXEDO_BUILD_NUMBER=${NEWEST_TUXEDO_TAG##*tux}
...
NEXT_TUXEDO_TAG_NUMBER=${NEXT_BASE_TAG_NUMBER}tux$((NEWEST_TUXEDO_BUILD_NUMBER+1))
...
if [[ ${DRY} ]]; then
    ...
    echo "    gbp dch --new-version \"${NEXT_TUXEDO_TAG_NUMBER//_/\~}\" --release --since \"${NEXT_BASE_TAG}\" --ignore-branch --spawn-editor \"never\""
    ...
    gbp dch --new-version "${NEXT_TUXEDO_TAG_NUMBER//_/\~}" --release --since "${NEXT_BASE_TAG}" --ignore-branch --spawn-editor "never"
    if [[ "$UBUNTU_KERNEL_BRANCH" == "master" ]]; then
