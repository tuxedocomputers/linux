...
if [[ "${UBUNTU_KERNEL_BRANCH}" == "master" ]]; then
    CURRENT_BASE_TAG=$(git describe --tags --match "Ubuntu-${UBUNTU_MAINLINE_VERSION}-*" --abbrev=0)
else
    CURRENT_BASE_TAG=$(git describe --tags --match "Ubuntu-${UBUNTU_KERNEL_BRANCH}-${UBUNTU_MAINLINE_VERSION}-*" --abbrev=0)
fi
CURRENT_BASE_TAG_NUMBER=${UBUNTU_MAINLINE_VERSION}-${CURRENT_BASE_TAG##*-"${UBUNTU_MAINLINE_VERSION}"-}
...
NEWEST_BASE_TAG_NUMBER=${UBUNTU_MAINLINE_VERSION}-${NEWEST_BASE_TAG##*-"${UBUNTU_MAINLINE_VERSION}"-}
